...
!> This solver is just a slightly edited version of the original Elmer/Ice floatation solver:
!! Here, grounded is (-) and floatation is (+), which is reversed compared to the
!! original.
...
! *  EDITED BY ALEX HUTH (ahuth@princeton.edu) in 2020
! *
...
SUBROUTINE Floatation( Model,Solver,dt,Transient )
!------------------------------------------------------------------------------
...
!  GLMAsk => VariableGet( Model % Mesh % Variables, 'GroundedMask')
  GLMAsk => VariableGet( Model % Mesh % Variables, 'Mask')  
  ...
  END IF
  BedVar => VariableGet( Model % Mesh % Variables, 'bed')
